...
主要用户是产品所有者本人。E-书痕（E-BookTrace）服务于在可信的 ZeroTier 私有网络中阅读 EPUB、整理划线与笔记，并把阅读所得持续沉淀为可复用材料的个人工作流。

## Product Purpose

E-书痕将 EPUB 原文阅读、划线、感悟、标签、书签、阅读进度、笔记管理、跨设备同步和 Markdown/Obsidian 导出连接成一条本地优先的路径。成功意味着用户可以从原文阅读开始，在不搬运上下文的情况下完成整理、恢复和导出。

## Positioning

E-书痕是主 EPUB 阅读器与 GPT 风格布局阅读器的组合，而不是聊天产品。GPT 风格阅读器保留书库、连续阅读、目录、主题和进度体验；产品不提供问答服务。ZeroTier 私有网络负责远程访问边界，应用本身不提供账号认证。
...
- 中文产品名为 E-书痕，英文产品名为 E-BookTrace。
- `marginalia` 仅保留为兼容存储、服务、脚本和路径标识，不再作为当前用户可见品牌。
- 界面简洁、清楚，避免无必要的信息噪声。
