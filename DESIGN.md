...
# Design System: E-书痕 / E-BookTrace

## Overview

**Creative North Star: "静谧书斋"**

E-书痕的界面像一间经过整理的私人书斋：温暖但不怀旧，精致但不炫耀。宣纸米白承载长时间阅读，墨松绿只在需要方向与确认时出现，衬线标题带来书卷气，系统无衬线字体保证工具界面的清晰和效率。
...
桌面端采用固定顶栏与可滚动工作区。书库以自适应卡片网格组织；阅读器以 AI 面板、正文阅读区和笔记面板构成三栏；笔记管理采用列表/详情双栏，列表和详情各自滚动。页面横向留白以 40px 为基准，面板间距主要为 16px，内部密度以 8px、12px、16px 递进。

在 1180px 以下，筛选区自适应换行；在 900px 以下，导航、页头、阅读面板与笔记管理转为单列，批量栏允许换行，详情变为固定全屏层；在 560px 以下，卡片内容和工具条进一步纵向堆叠。响应式变化优先保证文字完整和操作可达，不依赖横向压缩维持桌面结构。
...
顶栏是半透明温白纸面，使用细底边和极轻阴影。导航标签置于浅暖灰容器中，活动项回到白色纸面并使用墨松绿文字；移动端导航改为整行等宽标签。品牌标记是克制的方形墨松绿书章，不改成圆形应用气泡。
...
- **Do** 在窄屏把笔记列表/详情双栏重排为列表加全屏详情层，并允许中文按钮与状态文字换行。
- **Do** 对回收站使用明确的危险操作确认；批量栏、颜色文本名称和键盘焦点同时提供状态提示。
