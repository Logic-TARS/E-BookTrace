---
name: "Marginalia"
description: "一间把阅读、边注与笔记管理安静连接起来的数字书斋"
colors:
  canvas: "#f4f1ea"
  canvas-ink: "#ebe4d8"
  surface: "#fffdf8"
...
界面以精致克制为原则。层次来自纸张色差、细边框和低对比度环境阴影，而不是强烈玻璃效果或夸张悬浮。阅读内容是视觉中心；控件、状态和笔记管理工具以稳定、安静的方式退居其后。
...
- **Display**（700，响应式 30–46px，1.12）：仅用于书库与笔记管理工作台的一级标题。
...
产品采用“主界面 → 独立工作页”的页面层级。主界面以自适应书卡网格组织书库，并提供进入笔记管理的入口；点击书籍进入独立阅读页，点击笔记管理入口进入独立笔记管理页。阅读和笔记管理不互相直达，均通过“返回主界面”退出。阅读器以正文为中心，目录、书签与笔记面板按需收放；笔记管理工作台呈现素材与标签筛选、感悟编辑和删除，以及书籍笔记 Markdown 导出（写入服务端配置的 Obsidian 目录）；标签编辑位于阅读器的笔记弹窗。`book-chat` 保留为另一套 EPUB 阅读界面。页面横向留白以 40px 为基准，面板间距主要为 16px，内部密度以 8px、12px、16px 递进。

窄屏下，页头、阅读面板与笔记管理工作台逐步转为单列，操作区允许换行，阅读控制栏保持覆盖正文的浮层；在 560px 以下，卡片内容和工具条进一步纵向堆叠。响应式变化优先保证文字完整和操作可达，不依赖固定底部导航或横向压缩维持桌面结构。
...
品牌顶栏只出现在主界面，使用半透明温白纸面、细底边和极轻阴影。阅读页和笔记管理页不显示全局标签导航，而是在各自页头提供“返回主界面”。移动端同样不使用固定底部导航，让阅读内容占据完整安全视口。品牌标记是克制的方形墨松绿书章，不改成圆形应用气泡。
